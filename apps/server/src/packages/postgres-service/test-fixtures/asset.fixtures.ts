...
export function getAssetFixtures() {
  ...
  const uploadedAssets = [{
    contentType: 'image/gif', fileSize: 1024, filename: 'fake-img-1.gif',
  }, {
    contentType: 'image/jpg', fileSize: 2048, filename: 'fake-img-2.jpg',
  }];

  fixtures = uploadedAssets.map(uploadedAsset => {
    const { filename, contentType, fileSize } = uploadedAsset;
    return postgresService.getModels().Asset.fromUploadedAsset(contentType, fileSize, filename);
  });
  return fixtures;
}
